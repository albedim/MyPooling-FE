...
export const TripComponent = ({ time, owner_username, trip_id, step_id, username, departure_date, used_slots, slots, code }) => {
    ...
    <>
      <div className="box-shadow white-backgroundcolor border-radius-5 margin-left-40 height-240 width-340">
        ...
        <div className="space-between display-flex height-80">
          <div className="">
            <div className="display-flex align-center height-34 width-140"><h2 onClick={(e) => navigate("/profile/" + owner_username)} className="hover font-weight-500 font-size-18 margin-left-40 font-family">@{username}</h2></div>
            <div className="align-center display-flex height-24 width-140"><h4 className="font-size-14 font-weight-400 margin-left-40 gray-color font-family">{departure_date.substring(0, 10).split("-")[2] + "/" + departure_date.substring(0, 10).split("-")[1] + "/" + departure_date.substring(0, 10).split("-")[0].substring(2, 4) + ", " + time.substring(0,5)}</h4></div>
          </div>
